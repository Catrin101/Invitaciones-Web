/**
 * sections.js — Template Boda Editorial
 * Renderizadores específicos para el template de boda
...
// SPLASH
// ─────────────────────────────────────────────
export function renderSplash(couple) {
  const nameEl = document.querySelector('.splash__name');
  if (nameEl) nameEl.textContent = `${couple.bride} & ${couple.groom}`;
...
export function renderWelcome(welcome) {
  const el = document.getElementById('ch-welcome');
  if (!el || !welcome) return;

  el.innerHTML = `
    <div class="full-content">
      <span class="chapter-label">Bienvenida</span>
      ...
      <p class="nav-hint" style="margin-top:2rem;">Usa las flechas o desliza para continuar</p>
    </div>
  `;
}
...
export function renderCountdown(event, labels) {
  ...
  function tick() {
    ...
    setWithFlip(document.getElementById('cd-days'), pad(Math.floor(diff / 86400000)));
    setWithFlip(document.getElementById('cd-hours'), pad(Math.floor((diff % 86400000) / 3600000)));
    setWithFlip(document.getElementById('cd-minutes'), pad(Math.floor((diff % 3600000) / 60000)));
    setWithFlip(document.getElementById('cd-seconds'), pad(Math.floor((diff % 60000) / 1000)));
  }
...
export function renderDetails(event) {
  ...
  el.innerHTML = `
    <div class="split">
      <div class="split__photo">
        <img src="https://images.unsplash.com/photo-1520854221256-17451cc31bf?w=900&q=80"
